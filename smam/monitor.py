...
class Monitor:
    ...
    def print_fall_down(self, datetime, id, pos_x, pos_y, pos_z, model):
        print("  ---------------------------------------------------")
        print("    ADVERTENCIA")
        print("  ---------------------------------------------------")
        print("    Se ha detectado una combinación de parámetros de posicion x, y & z que indican una posible caída a las " + str(self.format_datetime(datetime)) + " en el adulto mayor que utiliza el dispositivo " + str(model) + ": " + str(id))
        print('    Posición X: %.2f' %float(pos_x))
        print('    Posición Y: %.2f' %float(pos_y))
        print('    Posición Z: %.2f' %float(pos_z))
        print("")
        print("")

    def print_take_medicine(self, datetime, id, medicine, model):
        print("  ---------------------------------------------------")
        print("    ALARMA")
        print("  ---------------------------------------------------")
        print("    Es hora de tomar el medicamento: " + str(medicine) + ", son las "+ str(self.format_datetime(datetime)) + " en el adulto mayor que utiliza el dispositivo " + str(model) + " : " + str(id))
        print("")
        print("")
